...
function Calculator() {
  ...
  const [hours, setHours] = useState<Record<HorizontalKey, number>>({
    core: 0,
    process: 0,
    data: 0,
    brand: 0,
  ...
  const [expectedMonthsMin, setExpectedMonthsMin] = useState(0);
  const [expectedMonthsMax, setExpectedMonthsMax] = useState(0);
  const [globalMultiplier, setGlobalMultiplier] = useState(1);
  const [includeWeekend, setIncludeWeekend] = useState(false);
  const [weekendPercentage, setWeekendPercentage] = useState(50);
  ...
  const calculations = useMemo(() => {
    ...
    (Object.keys(HORIZONTAL_LABELS) as HorizontalKey[]).forEach((key) => {
      ...
      const hourValue = baseValue * factor * globalMultiplier;
      const subtotal = hourValue * hours[key];
  ...
  const totalHours = useMemo(() => {
    return Object.values(hours).reduce((acc, h) => acc + h, 0);
  }, [hours]);

  // Cálculos do sumário do projeto
  const projectSummary = useMemo(() => {
    // Horas por pessoa por mês
    const hoursPerPersonPerMonth = totalHours / Math.max(teamSize, 1);
    ...
    if (includeWeekend && weekendPercentage > 0) {
      // Se incluir fim de semana, distribuir horas proporcionalmente
      // horasTotal = (diasUteis * horasDiaUtil) + (diasFds * horasDiaUtil * percentual)
      // horasTotal = horasDiaUtil * (diasUteis + diasFds * percentual)
      const effectiveDays = workDaysPerMonth + (weekendDaysPerMonth * (weekendPercentage / 100));
      dailyHoursWeekday = hoursPerPersonPerMonth / effectiveDays;
      dailyHoursWeekend = dailyHoursWeekday * (weekendPercentage / 100);
    } else {
    ...
    const totalProjectHoursMin = totalHours * expectedMonthsMin;
    const totalProjectHoursMax = totalHours * expectedMonthsMax;
    const avgTotalProjectHours = (totalProjectHoursMin + totalProjectHoursMax) / 2;
    
    // Horas de Core por semana (4 semanas por mês)
    const coreHoursPerWeek = hours.core / 4;
    ...
    const totalRevenueMin = calculations.total * expectedMonthsMin;
    const totalRevenueMax = calculations.total * expectedMonthsMax;
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => setGlobalMultiplier(Math.max(0.1, Number(e.target.value) || 1))}
                    ...
                    <tr key={key} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-4 text-center">
                        <input
                          ...
                          min="0"
                          value={hours[key] || ""}
                          onChange={(e) => setHours({ ...hours, [key]: Number(e.target.value) || 0 })}
                          placeholder="0"
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <input
                      ...
                      onChange={(e) => setExpectedMonthsMin(Number(e.target.value) || 0)}
                      placeholder="0"
                      className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                    />
                    <span className="text-xs text-muted-foreground mt-1 block">Mínimo</span>
                  </div>
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      onChange={(e) => setExpectedMonthsMax(Number(e.target.value) || 0)}
                      placeholder="0"
                      className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                    />
                    <span className="text-xs text-muted-foreground mt-1 block">Máximo</span>
                  </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="p-4 rounded-xl border border-border bg-background">
                  ...
                  {includeWeekend && (
                    <div className="mt-2 flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => setWeekendPercentage(Math.min(100, Math.max(0, Number(e.target.value) || 0)))}
                ...
                <div className="mt-3 pt-3 border-t border-border space-y-1">
                  ...
                  {includeWeekend && weekendPercentage > 0 && (
                    <p className="text-sm text-muted-foreground">
              ...
              {(expectedMonthsMin > 0 || expectedMonthsMax > 0) && (
                <div className="p-4 rounded-xl border border-border bg-background">
                  <div className="flex items-center gap-2 text-muted-foreground mb-2">
